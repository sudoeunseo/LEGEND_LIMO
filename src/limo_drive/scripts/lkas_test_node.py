...
class LKAS:
    def __init__(self):
        ...
        # -----------------------------
        # OpenCV imshow 디버그 옵션
        # -----------------------------
        self.debug_view = True          # True면 imshow 활성화
        self.debug_wait = 1             # waitKey(ms). 1이면 실시간
        self.debug_every = 1            # N프레임마다 화면 갱신 (부하 줄이기)
    ...
    def img_binary(self, blend_line):
        ...
        # 하단 구간(y_start~h) 중앙 밴드를 0으로 제거
        binary_line[y_start:h, x0:x1] = 0

        # (선택) 상단 체크무늬/가로 라인(횡단선)도 방해되면 제거
        # top_cut = int(h * 0.12)            # 필요하면 0.08~0.18 조절
        # binary_line[0:top_cut, :] = 0
    ...
    def img_CB(self, data):
        ...
        if self.debug_view and (self._frame_count % self.debug_every == 0):
            # binary는 0/1이라 보기 좋게 0/255로 스케일
